     PROGRAM
!This version replaces most Messages with the Task Dialog. That code is all at the end.
!It also has a Shell Auto Complete for the Generate Classes Path name. I prefer that to the FileDialog()
...
!EndRegion Ancient Documentation
     ...
     MAP 
TskDlg_Welcome PROCEDURE()  !Task Dialog when program Starts
TskDlg_GenerateCode PROCEDURE(),BYTE  !Task Dialog asks what code to Generate 1=Map 2=Ron 3=Class
TskDlg_2Buttons PROCEDURE(STRING Instrx, STRING Content, STRING Caption,STRING Btn1, STRING Btn2, BYTE Btn2IsCancel=0, BYTE BtnDefault=1),BYTE
TskDlg_AddExportQSkipSym PROCEDURE(STRING SkipSymbolName, *BYTE OmtIncAllDllX),BYTE   !Msg about symbols like DllGetVersion that are skipped
TskDlg_AddExportQSkipDup PROCEDURE(STRING DupSymbolName, *BYTE OmtIncAllDups),BYTE   !Msg about Duplcate Symbol
        ReadExecutable
...
window WINDOW('MGLibMkr Plus with Task Dialog by Carl Barnes for CIDC 2019'),AT(,,288,257),CENTER,GRAY,IMM,SYSTEM,ICON('LIBRARY.ICO'),FONT('Segoe UI',9),ALRT(CtrlShiftD), |
            DROPID('~FILE'),RESIZE
...
PreAccept            ROUTINE
   ...
   ?TOOLBAR1{PROP:Color}=0d5e8eeh !Diff color #eee8d5  look diff for TaskDlg version. From Solarized      
...
!   Message('TD Generate Pick=' & TskDlg_GenerateCode()) ; halt !Easy Test 
!   Message('Selected=' & TskDlg_2Buttons('View the MAP file with Notepad?','FileName','View Map','View','Cancel',1)) ; halt
   IF ~RECORDS(ExportQ) THEN TskDlg_Welcome().  !If command line loaded then no welcome
!------------------------------------------------------
...
FileAdded            ROUTINE
  SetCursor(CURSOR:Wait)
  IF UPPER(RIGHT(FileName,4))='.LIB' THEN  !01/02/22
    ReadLib()
  ELSE
    ReadExecutable()
  END
...
!                      BUTTON:Abort+BUTTON:Ignore)=BUTTON:Abort THEN 
          IF TskDlg_2Buttons('LibMaker cannot load 64-bit files.', |
                           'The Header on this file has the 64-bit Magic Number 20Bh.' & |
                           '<13,10><13,10>File: ' & CLIP(name(exefile)), |
                           '64-Bit Binary?','Load 64-bit DLL','Skip DLL',1,2) <> 1 THEN 
             CLOSE(EXEfile)
             RETURN
          END
       END      
   ...
   ELSIF 1=TskDlg_2Buttons('This is a 16-bit DLL.', |
                           'File: ' & CLIP(name(exefile)),'16-Bit Binary?','Load 16-bit DLL','Skip DLL',1,2) THEN  ! dos_magic <> 'MZ
...
MaxSymLen   LONG
   ...
   IF ~FileDialog('Save Clarion Map definition as ...', FileName, |
       'CW Source and Includes (*.clw,*.inc)|*.clw;*.inc|Source Only|*.clw|Includes Only|*.inc|All|*.*',|
        FILE:Save+FILE:LongName)  !TODO Consider FILE:AddExtension, but then only 1 EXT
       RETURN
    END
   ...
   IF TskDlg_2Buttons('View the MAP file with Notepad?',FileName,'Generated Map','View Map','Cancel',1)=1 THEN 
...
QRec           LONG,AUTO
ClassPath      STRING(FILE:MaxFilePath),STATIC
ClassPrefix    STRING(10),STATIC
...
ClsWindow WINDOW('Generate Classes'),AT(,,323,96),GRAY,SYSTEM,FONT('Segoe UI',9,,FONT:regular)
        PROMPT('&Name Prefix:'),AT(5,5),USE(?ClsPrfx:Pmt)
        ENTRY(@s10),AT(51,4,,11),USE(ClassPrefix)
        PROMPT('Target &Path:'),AT(5,20),USE(?ClsPath:Pmt)
        ENTRY(@s255),AT(51,20,235,11),USE(ClassPath)
        PROMPT('This will generate a Class INC and CLW file for each Libary in the list with a wrapper method for each s' & |
                'ymbol. The files are placed on the path specified.'),AT(46,67,231,19),USE(?SubSummary)
        BUTTON('Pic&k'),AT(291,20,27,12),USE(?ClsPickBtn)
        BUTTON('&Generate'),AT(108,41,48,15),USE(?ClsGenBtn)
        BUTTON('&Cancel'),AT(167,41,48,15),USE(?ClsCloseBtn),STD(STD:Close)
        STRING('(For Class Name and File Name. Use File/Label Valid Characters)'),AT(108,5),USE(?STRING1)
    END
   CODE
   IF ~ClassPath   THEN ClassPath=GetIni('Class','Path',,qINI_File).
   IF ~ClassPrefix THEN ClassPrefix=GetIni('Class','Prefix','ct',qINI_File).
   OPEN(ClsWindow) 
   SHAutoComplete(?ClassPath{PROP:Handle}, 20h)
   ACCEPT
      CASE ACCEPTED()
      OF ?ClsGenBtn
         IF ~ExistsFolder(ClassPath) THEN SELECT(?ClassPath) ; BEEP ; CYCLE.
         IF ~ClassPrefix THEN SELECT(?ClassPrefix) ; BEEP ; CYCLE.
         GenDir=CLIP(ClassPath)
         GenPfx=CLIP(ClassPrefix)
         PUTIni('Class','Path'  ,ClassPath,qINI_File)
         PUTIni('Class','Prefix',ClassPrefix,qINI_File) 
         BREAK
      OF ?ClsPickBtn
         FileDialog('Select Target Folder to Generate the Classes?', ClassPath,,FILE:KeepDir + FILE:NoError + FILE:LongName + FILE:Directory)
         DISPLAY
      END
   END
   Close(ClsWindow)
   IF ~GenDir THEN RETURN.
...
!   DO PromptForFolder !may return
   ...
   SETCURSOR()
   !IF MES SAGE('Would you like to open Explorer?||'&GenDir,'Classes Generated',ICON:Question,Button:Yes+Button:No)=Button:Yes
   IF TskDlg_2Buttons('Open Explorer to View Folder?',GenDir,'Classes Generated','View Folder','Cancel',1)=1 THEN 
...
SetNames            ROUTINE
   ...
   CLW_Name       = GenPfx & ModuleBaseName & '.clw'
   INC_Name       = GenPfx & ModuleBaseName & '.inc'
   ClassName      = GenPfx & ModuleBaseName 
...
!          END !CASE
          SkipIt = 1 
          CASE TskDlg_AddExportQSkipSym(SymName,OmtIncAllDllX)
          !OF 1 ; SkipIt=1 
          OF 2 ; SkipIt=0             ! Name: Include         
          END          
          RETURN SkipIt
    END 
    SaveExportQ = ExportQ
...
!        END !CASE        
        CASE TskDlg_AddExportQSkipDup(SymName,OmtIncAllDups)
        OF 1 ; SkipIt=1 
      ! OF 2                     ! Name: Include         
        END
    END
...
NoMsgDate   LONG    
    CODE
    IF BAND(KEYSTATE(),0200h) THEN          !Hold Control to reset Welcome 
       PUTINI('Welcome','NoMsg',TODAY()-1,qINI_FILE)
    END
    NoMsgDate=GetIni('Welcome','NoMsg',0,qINI_FILE)
    IF NoMsgDate >= TODAY() THEN RETURN.

    TD.Init()
    TD.DevBugsAreReported(1,1)  ! 1=get message for bad Code, ,1=GLobally ON for ALL
    td.dd.Caption='MG LibMaker Minus with Task Dialog'
    TD.dd.Instruction='Welcome to LibMaker Minus with Task Dialog'
    TD.dd.Content='MG LibMaker Minus is an example created by Carl Barnes for CIDC 2019 ' & |
                'based on MG LibMaker. It is a useful utility with far more features than the shipping LibMaker: ' & |
                'Subtraction, Searching, Tagging, WinSxS...'
                
    TD.DD.ExpandSeeMoreLabel='See More About Features'
    TD.DD.ExpandSeeLessLabel='See Less About Features'
    TD.DD.FLag.ExpandInFooter=1
    TD.DD.ExpandedText='To quickly load common DLLs Right-Click on the Add / Subtract buttons for a popup ' & |
                'menu that allows loading Windows or Clarion files without having to ' & |
                'navigate the in the file dialog.' & |
                '<13,10,13,10>Subtraction allows easily removing duplicates of Clarion''s WIN32.LIB. It also ' & |
                'allows seeing "What''s New" by loading a new DLL and subtracting the older one. ' & |'
                'E.g. Load a Windows 8 DLL and subtract the Windows 7 DLL.'
...
!    TD.dd.Flag.Hyperlinks=1
!    TD.dd.FooterText='More information at <A HREF="http://www.cidc2019.com">CIDC2019.com</A> ' & |
!                                     'and <A HREF="http://www.ClarionLive.com">www.ClarionLive.com</A>' 
    TD.dd.FooterText='More information at ' & TD.HRefText('http://www.cidc2019.com','CIDC2019.com') & |
                                    ' and ' & TD.HRefText('http://www.ClarionLive.com','www.ClarionLive.com') & '.'
    !----------- Hyperlinks ----
    TD.dd.CheckboxLabel='Do Not Show Welcome Message'
    !td.dd.flag.Size2ContentOFF=1   !Changed to OFF swicth because it should always be ON else too narrow.
    td.Btns_OYNRCL('L') 
    td.dd.flag.CancelAllowed=1
    td.dd.flag.Position2Window=1
    TD.DD.Width=220
    TD.Show()
    IF ~TD.Result.Checked THEN RETURN.
    !------------------------------- 2nd Dlg -------
    
    !Init(<STRING Instruction_1>,<STRING Content_2>,<STRING Caption_3>,<LONG Icon_4>,<STRING Btns_OYNRCL_5>,<STRING Default_TDBtn_OrOYNRCL_6>)     !CLEARS(TDlg)
    TD.Init('You Checked "Do Not Show Welcome Message"?',|
            'Please confirm you do not wish to see this message. It will reappear tomorrow.', |
            'No Welcome?')
     
    TD.DD.ExpandSeeMoreLabel='Technical Information'      
    TD.DD.ExpandedText='Turning off Welcome is done by writing Date to [Welcome] NoMsg=' & TODAY() &|
            '<13,10>in ' & CLIP(qINI_FILE) &|
            '<13,10>Holding down Ctrl when running will reactivate the Welcome message.'
    ...
    TD.BtnCustomAdd(11,'&No Welcome')
    TD.BtnCustomAdd(12,'&Show Welcome',1)
    TD.DD.Flag.CancelAllowed=1
    TD.Show()
    IF TD.Result.ButtonID=11 THEN
       PUTINI('Welcome','NoMsg',TODAY(),qINI_FILE)
       RUN('Notepad ' & qINI_FILE)
    END
!=====================================================================
TskDlg_GenerateCode PROCEDURE()!,BYTE  !Task Dialog asks what code to Generate 1=Map 2=Ron 3=Class
TD  CBTaskDlgClass
...
!--- Radios do not seem as nice
    TD.Init('What Code would you like to Generate?',|
            ,|
            'Generate Code') 
    
    TD.DD.ExpandedText='MAP will provide prototypes of the procedures. Ron''s MAP format is ... for Ron.' & |
            '<13,10>Select Classes create ... Class files.'

    TD.RadioAdd(1, 'MAP',1)   !Default radio
    TD.RadioAdd(2, 'MAP - Ron''s Format')
    TD.RadioAdd(3, 'Classes')
    TD.BtnCustomAdd(101,'Generate')
    TD.Btns_OYNRCL('C','C')     !Cancel and it is the Default
    TD.DD.Flag.CancelAllowed=1  !Red X in Corner 
    TD.Show()
    IF TD.Result.ButtonID=101 THEN  !Clicked Generate Button
       Ret123 = TD.Result.RadioID
    END
    return Ret123
!======================================================
!A simple way to change this program from Message() to TD. There simple be a Message() like Procedure
TskDlg_2Buttons PROCEDURE(STRING Instrx, STRING Content, STRING Caption,STRING Btn1, STRING Btn2, BYTE Btn2IsCancel=0, BYTE BtnDefault=1)!,BYTE
TD  CBTaskDlgClass
Ret12 BYTE
    CODE
    TD.Init(Instrx,Content,Caption)
    TD.BtnCustomAdd(101,Btn1,CHOOSE(BtnDefault<>2))
    TD.BtnCustomAdd(102,Btn2,CHOOSE(BtnDefault =2))
    IF Btn2IsCancel THEN TD.DD.FLAG.CancelAllowed=1.
    TD.Show()
    Ret12 = TD.Result.ButtonID
    IF Ret12=TD_ID_CANCEL THEN Ret12=102 .
    RETURN CHOOSE(Ret12=101,1,2) 
!======================================================
TskDlg_AddExportQSkipSym PROCEDURE(STRING SymName, *BYTE OmtIncAllDllX)!,BYTE   !Msg about symbols like DllGetVersion that are skipped
TD  CBTaskDlgClass
Ret12 BYTE
    CODE
    TD.Init()
    ...
    return Ret12
!--------------------    
TskDlg_AddExportQSkipDup PROCEDURE(STRING SymName, *BYTE OmtIncAllDups)!,BYTE   !Msg about Duplcate Symbol
TD  CBTaskDlgClass
Ret12 BYTE
    CODE
    IF OmtIncAllDups THEN RETURN(2-OmtIncAllDups).  !Caller should have done with .... but could work like this
    TD.Init()
    TD.dd.Caption='Duplicate Symbol'
    TD.dd.Instruction='Duplicate Symbol: ' & SymName
    TD.dd.Content='A Symbol being added already exists. It usually will cause'&|
             ' Linker errors if there are duplicate symbol names.'&|
             '<13,10><13,10>     Symbol: '& CLIP(ExportQ.symbol) &|
             '<13,10>     Module: '& CLIP(ExportQ.module)
    TD.BtnCustomAdd(101,'&Omit Duplicate')
    TD.BtnCustomAdd(102,'&Include Duplicate')
    TD.DD.CheckboxLabel='&Apply to All Duplicate Symbols<13,10> {9}(until program closes)'
    TD.DD.Width=200
    TD.Show()
    Ret12=TD.Result.ButtonID-100
    IF TD.Result.Checked THEN OmtIncAllDups=Ret12 .
    return Ret12