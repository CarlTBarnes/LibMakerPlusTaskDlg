...
window WINDOW('MGLibMkr'),AT(,,288,257),CENTER,GRAY,IMM,SYSTEM,ICON('LIBRARY.ICO'),FONT('Segoe UI',9),ALRT(CtrlShiftD), |
            DROPID('~FILE'),RESIZE
...
Accepted:SubtractFile  ROUTINE
    SubtractFileLIB() ; ExportQ_to_FilterQ() ; DO EnableDisable ; SELECT(?LIST1) ; DO Set_DisplayCount
...
IsADD BOOL
   ...
   OF ?AddFile OROF ?SubtractFile
      ...
      OF 1 ; IF ~FileNamePopupSystem32(FileNmRef) THEN EXIT. ; POST(EVENT:Accepted,F)
      OF 2 ; Filename=System32 ; POST(EVENT:Accepted,F)
      OF 3 ; IF ~WinSxSPicker(FileNmRef) THEN EXIT. ; POST(EVENT:Accepted,F)      
      OF 4 ; IF ~FileNamePopupClarion(FileNmRef) THEN EXIT. ; POST(EVENT:Accepted,F)
      OF 5 ; DO LoadWin32LibDllsRtn
      END
   OF ?List1            ! ?TAB:All
